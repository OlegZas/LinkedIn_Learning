...
-- SELECT e.employeeId, count(M.modelid) AS UNITSSOLD
-- FROM employee E 
-- INNER JOIN SALES S on e.employeeId = S.employeeId
-- INNER JOIN INVENTORY I ON S.INVENTORYID = I.INVENTORYID 
-- INNER JOIN MODEL M ON I.MODELID = M.MODELID 
...
-- 12. GENERATE A REPORT SHOWING THE NUMBER OF CARS SOLD THIS MONTH AND LAST MONTH: 
WITH MONTHLYUNITS AS (
  SELECT  STRFTIME('%Y-%m',soldDate) as MONTH, COUNT(salesId) AS UNITSSOLD
  FROM SALES S 
  GROUP BY STRFTIME('%Y-%m',soldDate)
  )
  SELECT MONTH, UNITSSOLD,LAG(UNITSSOLD,1,0)OVER(ORDER BY MONTH) AS TRAILINGSALES
  FROM MONTHLYUNITS
  ORDER BY MONTH 
